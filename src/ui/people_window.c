...
static uint16_t get_num_rows_callback(MenuLayer *menu_layer, uint16_t section_index, void *context) {
  return NB_OF_PEOPLES;
}

static void draw_row_callback(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *context) {
  menu_cell_basic_draw(ctx, cell_layer, PEOPLE_TEXTS[cell_index->row], NULL, NULL);
}

static void draw_header_callback(GContext *ctx, const Layer *cell_layer,  uint16_t section_index, void *context) {
  menu_cell_basic_header_draw(ctx, cell_layer, PBL_IF_ROUND_ELSE("            Who are you with?","Who are you with?"));
}
...
static void select_callback(struct MenuLayer *menu_layer, MenuIndex *cell_index, void *callback_context) {
  if(cell_index->row == NB_OF_PEOPLES-1){
    if(!s_dictation_session)
...
void show_people_window(PeopleChangedHandler handler) {
  s_handler = handler;
  s_other[0] = 0;
